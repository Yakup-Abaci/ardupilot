...
void GCS_MAVLINK_Plane::send_position_target_local_ned()
{

    const ModeGuided::SubMode guided_mode = plane.mode_guided.submode();
    Vector3f target_pos;
    Vector3f target_vel;
    ...
    case ModeGuided::SubMode::Pos:
        ...
        target_pos = plane.mode_guided.get_target_pos().tofloat() * 0.01; // convert to metres
        break;
    case ModeGuided::SubMode::PosVelAccel:
        ...
        target_pos = plane.mode_guided.get_target_pos().tofloat() * 0.01; // convert to metres
        target_vel = plane.mode_guided.get_target_vel() * 0.01f; // convert to metres/s
        target_accel = plane.mode_guided.get_target_accel() * 0.01f; // convert to metres/s/s
        break;
...
MAV_RESULT GCS_MAVLINK_Plane::handle_command_int_guided_slew_commands(const mavlink_command_int_t &packet)
{
    ...
    case MAV_CMD_GUIDED_CHANGE_ALTITUDE:
    {
        ...
        {
            if (is_equal(plane.guided_state.target_alt, new_target_alt_rel))
        ...
        {
            if (is_equal(plane.guided_state.target_alt, new_target_alt))
            { // compare two floats as near-enough
                // no need to process any new packet/s with the same ALT any further, if we are already doing it.
                return MAV_RESULT_ACCEPTED;
            }
...
bool GCS_MAVLINK_Plane::sane_vel_or_acc_vector(const Vector3f &vec) const
{
...
void GCS_MAVLINK_Plane::handle_set_attitude_target(const mavlink_message_t &msg)
{
    ...
    if (att_target.type_mask & 0b01000000)
    { // throttle
...
void GCS_MAVLINK_Plane::handle_set_position_target_local_ned(const mavlink_message_t &msg)
{
    ...
    if (!pos_ignore)
    {
        ...
        if (packet.coordinate_frame == MAV_FRAME_LOCAL_OFFSET_NED ||
            ...
            packet.coordinate_frame == MAV_FRAME_BODY_OFFSET_NED)
        {
            pos_vector += plane.inertial_nav.get_position_neu_cm();
        }
    }
    ...
    {
        plane.mode_guided.set_destination_posvelaccel(pos_vector, vel_vector, accel_vector, !yaw_ignore, yaw_cd, !yaw_rate_ignore, yaw_rate_cds, yaw_relative);
    }
    else if (pos_ignore && !vel_ignore)
    {
        plane.mode_guided.set_velaccel(vel_vector, accel_vector, !yaw_ignore, yaw_cd, !yaw_rate_ignore, yaw_rate_cds, yaw_relative);
    }
    else if (pos_ignore && vel_ignore && !acc_ignore)
    {
        plane.mode_guided.set_accel(accel_vector, !yaw_ignore, yaw_cd, !yaw_rate_ignore, yaw_rate_cds, yaw_relative);
    }
    else if (!pos_ignore && vel_ignore && acc_ignore)
    {
        plane.mode_guided.set_destination(pos_vector, !yaw_ignore, yaw_cd, !yaw_rate_ignore, yaw_rate_cds, yaw_relative, false);
    }
    else
    {
        // input is not valid so stop
    ...
    gcs().send_text(MAV_SEVERITY_INFO, "gidiyor emmoğlu ama nasil bilmiyorum %.1f",
