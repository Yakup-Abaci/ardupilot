...
float Plane::calc_speed_scaler(void)
{
    ...
    { // scaling is suppressed during climb phase of automatic takeoffs with no airspeed sensor being used due to problems with inaccurate airspeed estimates
...
float Plane::stabilize_roll_get_roll_out()
{
    ...
    if (!quadplane.use_fw_attitude_controllers())
    {
        ...
        {
            const float mc_angR = quadplane.attitude_control->get_angle_roll_p().kP() * quadplane.attitude_control->get_last_angle_P_scale().x;
...
float Plane::stabilize_pitch_get_pitch_out()
{
    ...
    if (!quadplane.use_fw_attitude_controllers())
    {
        ...
        if (quadplane.option_is_set(QuadPlane::OPTION::SCALE_FF_ANGLE_P))
        {
            const float mc_angP = quadplane.attitude_control->get_angle_pitch_p().kP() * quadplane.attitude_control->get_last_angle_P_scale().y;
